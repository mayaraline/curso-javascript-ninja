(function(){
    ...
    */
    ...
    function hasClass(markup, cssClass) {
        var regex = new RegExp('class=["\']([\\w\\s]+)?' + cssClass + '["\']?' , 'gmi');
        return regex.test(markup);
    }
    var testClasses = ['container', 'text', 'date', 'excerpt', 'main'];

    testClasses.forEach(function(item, index, array) {
