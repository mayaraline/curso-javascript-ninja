...
carro adicionaPessoas = function(numeroPessoas) {
    ...
    if ((carro.quantidadePessoas < carro.assentos) && ((carro.quantidadePessoas + numeroPessoas) > carro.assentos)) {

        var complemento = (carro.assentos - carro.numeroPessoas) === 1 ? 'pessoa' : 'pessoas';

        return `Só cabem mais ${carro.assentos - carro.numeroPessoas} ${complemento}`;
    }

    return `Já temos ${carro.quantidadePessoas} ${complemento} no carro!`;
}
