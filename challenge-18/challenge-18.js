...
function cleanCPF(cpf){
    var cpfCleanRegex = /\D/g
    return cpf.replace(cpfCleanRegex, '');
}
...
var cpfPattern = /(\d\d\d)(\d\d\d)(\d\d\d)(\d\d)/;
function formatCPF (regex, part1, part2, part3, part4){
    return `${part1}.${part2}.${part3}-${part4}`;
}

console.log(`${cleanCPF("049-214 3421-1").replace(cpfPattern, formatCPF)}`);
console.log(`${cleanCPF("210.458.522-05").replace(cpfPattern, formatCPF)}`);
console.log(`${cleanCPF("735 500 794 - 22").replace(cpfPattern, formatCPF)}`);
console.log(`${cleanCPF("101.123-131x32").replace(cpfPattern, formatCPF)}`);
...
var months = /ju\w+/g;
console.log('Os meses de janeiro, junho e julho começam com a letra j.'.match(months));
...
pattern = /(<(\w+)>)([^<|^>]+)(<\/\w+>)/g;
    ...
    function formatText(regex, part1, part2, part3, part4) {
        return `${part1}O texto dentro da tag "${part2}" é "${part3}" ${part4}\n`;
    }
    console.log(text.replace(pattern, formatText));
